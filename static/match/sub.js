...
$(window).on('load', function() { 
	...
	$('input[name="player_out"]').change(function() {

		side = $(this).attr('side')
		var nU19 = 0 
		var nU21 = 0 

		$('input[name="player_out"][side='+side+']').each(function() {
			el = $(this)
		...
		$("input[name=player_out][side="+side+"]").closest('div').css({"border-style":""})

		$(this).closest('div').css({"border-style":"solid", "border-color":"yellow"})
		age = $(this).data('player-age')

		$('input[name=player_in][side='+side+']').each(function() {
	...
	$('input[name=player_in]').change(function() {
		side = $(this).attr('side')
		$('input[name=player_in][side='+side+']').filter("input:enabled").closest('div').css({"border-style":""})
