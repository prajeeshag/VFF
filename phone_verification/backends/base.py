...
class BaseBackend:
    ...
    def generate_security_code(cls):
        ...
        token_length = getattr(
            django_settings, 'PHONE_VERIFICATION_TOKEN_LENGTH', DEFAULT_TOKEN_LENGTH)
    ...
    def generate_session_token(cls, phone_number):
        ...
        """
        data = ','.join([str(ele) for ele in ["phone_number",
                                              phone_number,
                                              "nonce", random.random()]])
    ...
    def create_security_code_and_session_token(self, number, session_token=None):
        """
        ...
        if stored_verification is None or self.check_security_code_expiry(stored_verification):
